...
def conv_layer(input, filter, kernel, stride, padding='SAME', layer_name="conv"):
    with tf.name_scope(layer_name):
        network = tf.keras.layers.Conv2D(inputs=input, use_bias=False, filters=filter,
...
def Average_pooling(x, pool_size=(2, 2), stride=2, padding='SAME'):
    return tf.keras.layers.AveragePooling2D(inputs=x, pool_size=pool_size, strides=stride, padding=padding)
...
def Fully_connected(x, units=class_num, layer_name='fully_connected'):
    with tf.name_scope(layer_name):
        return tf.keras.layers.Dense(inputs=x, use_bias=False, units=units)
...
training_flag = tf.keras.backend.placeholder(dtype=tf.bool)
